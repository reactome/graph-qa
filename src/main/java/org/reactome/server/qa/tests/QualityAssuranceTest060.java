...
//@QATest
@Deprecated
public class QualityAssuranceTest060 extends QualityAssuranceAbstract {

    @Override
    public String getName() {
        return "DuplicatedInferredCandidateSets";
    }
    ...
    String getQuery() {
        return " MATCH (n1c)<-[:compartment]-(n1:CandidateSet)-[:hasMember|hasCandidate]->(:PhysicalEntity)<-[:hasMember|hasCandidate]-(n2:CandidateSet)-[:compartment]->(n2c) " +
                "WHERE NOT ()-[:inferredTo]->(n1) AND ()-[:inferredTo]->(n2) AND NOT n1 = n2 AND n1c = n2c " +
                "WITH DISTINCT n1, n2 " +
