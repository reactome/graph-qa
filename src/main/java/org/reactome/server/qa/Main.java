...
public class Main {
    ...
    private static void printReports(List<Report> reports, long time) {
        ...
        System.out.println(String.format("\nQA finished. %s test%s generated reports (%s)", c, c == 1 ? "" : "s", getTimeFormatted(time)));
    }
    ...
    private static void initialise(JSAPResult config) {
        ...
        if (VERBOSE) {  //Report test initialisation
            ...
            long t = tests.size();
            System.out.println(String.format("\r\t> %d test%s found: ", t, t == 1 ? "" : "s"));
            long r = sortedTests.size();
            System.out.println(String.format("\t\t-%3d test%s active", r, r == 1 ? "" : "s"));
